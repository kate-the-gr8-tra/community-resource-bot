...
class MyCog(ext_commands.Cog):
    ...
    @app_commands.command(
        name="register",
        description="Register your pronouns and info (note: a link will override other info)",
    )
    @app_commands.describe(
        link="Your pronouns.page link",
    ...
    async def verify(
        ...
        if link:
            ...
            if pronouns and not found_valid_link:
                user_data = await MyCog.try_manual_info(
                    discord_user_id, discord_username, server_id, name, pronouns, age
                )
    ...
    @app_commands.command(
        name="edit_info", description="Register your pronouns and info"
    )
    @app_commands.describe(
        link="Your new pronouns.page link",
    ...
    async def edit_info(
        ...
        if updated_data and (not updated_data["age"] or updated_data["age"] > 0):
            try:
                ...
                await ctx.followup.send(
                    f"""You want to update your information to: \n
                ...
                def check(message: discord.Message):
                    return message.author == ctx.user and message.content.lower() in [
                        "yes",
                        "no",
                    ]
                ...
                try:
                    ...
                    if response.content.lower() == "no":
                        await ctx.followup.send(
                            "No changes were made to your information."
                        )
                        return
                except TimeoutError:
    ...
    @app_commands.command(
        name="delete_info", description="""Deletes user information from the registry"""
    )
    async def delete_info(self, ctx: discord.Interaction):
        ...
        try:
            ...
            await ctx.followup.send(
                "Would you like to delete your information? (yes/no)"
            )
            ...
            connection.commit()
            await ctx.followup.send(
                "Your stored information has been removed. "
    ...
    @app_commands.command(
        name="send_info",
        description="""Sends user information and crafts an example sentence using the user's name and pronoun information""",
    )
    ...
    @app_commands.command(
        name="ping", description="Check if bot is alive and its latency"
    )
